...
missing. Ground first per workflow step 1 (mirror the nearest existing
controller/service/entity). **Match the repo's established style** (controllers
vs minimal APIs, `Startup.cs` vs top-level `Program.cs`) — don't introduce the
other mid-task.
